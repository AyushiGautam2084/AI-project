...
def astar_solver(maze, start, end):
    priority_queue = [(heuristic(start[0], start[1], end[0], end[1]) + 0, 0, start)]
    cost_so_far = {start: 0}
    came_from = {}
        ...
        for dx, dy in directions:
            ...
            if is_valid_move(maze, new_x, new_y):
                ...
                if (new_x, new_y) not in cost_so_far or new_cost < cost_so_far[(new_x, new_y)]:
                    ...
                    heapq.heappush(priority_queue, (heuristic(new_x, new_y, end[0], end[1]) + new_cost, new_cost, (new_x, new_y)))
...
def main():
    ...
    path = astar_solver(maze, start, end)
